...
"""
    ...
    def bootstrap(self):
        ...
        try:
            ...
            raise cdist.Error("chrooted apt-get update failed (root "
                              "priviliges required)")
    ...
    def create_initramfs(self):
        ...
        cmd = ("cd {target_dir}; find . -print0 | cpio --null -o "
               "--format=newc | gzip -9 > {out_file}").format(
                  target_dir=self.target_dir, out_file=out_file)
        ...
        if replace_manifest:
            ...
            self.initial_manifest = ("{default}\n# User supplied "
                                     "manifest\n{user}").format(
                                         ...
                                         user=user_initial_manifest_content)
    ...
    def commandline(cls, args):
        self = cls(target_dir=args.target_dir[0], arch=args.arch)

        # read initial manifest first - it may come from stdin
        if args.config:
            self.setup_initial_manifest(
                args.initial_manifest, args.replace_manifest)
